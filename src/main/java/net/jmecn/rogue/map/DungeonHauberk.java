...
public class DungeonHauberk extends MapCreator {
	...
	static Vector2[] Direction = { new Vector2(1, 0), new Vector2(0, 1), new Vector2(-1, 0), new Vector2(0, -1) };
	...
	private void growMaze(Vector2 start) {
		...
		while (!cells.isEmpty()) {
			...
			if (!unmadeCells.isEmpty()) {
				...
				cells.add(new Vector2(cell.x + dir.x * 2, cell.y + dir.y * 2));
				lastDir = dir;
			} else {
	...
	class Graph {
		...
		void union(int x, int y, Vector2 p) {
			if (matrix[x][y] == null) {
				Edge e = new Edge();
	...
	private void connectRegions() {
		...
		for (int y = 1; y < height - 1; y++) {
			for (int x = 1; x < width - 1; x++) {
				...
				if (a != -1 && b != -1) {
					graph.union(a, b, new Vector2(x, y));
				}
			}
		}
...
//					}
				...
				List<Vector2> points = e.points;
				// Carve the connection.
				len = points.size();
				Vector2 p = points.get(nextInt(len));
				addJunction(p);
				points.clear();
			}
	 ...
	 * @return
	 */
	private boolean canCarve(Vector2 pos, Vector2 direction) {
		// Must end in bounds.
