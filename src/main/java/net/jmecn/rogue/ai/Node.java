...
public class Node implements Comparable<Node> {
	public Vector2 _Pos; // position of the node
	...
	// initialize the NOde
	public Node(Vector2 _Pos, int col, int row) {
		this._Pos = _Pos;
		this.col = col;
		this.row = row;
	}
	...
	public LinkedList<Node> getLimit() {
		...
		int y = _Pos.y;
		if (y > 0) {
			limit.add(new Node(new Vector2(x, y - 1), col, row)); // up
		}
		if (y < row) {
			limit.add(new Node(new Vector2(x, y + 1), col, row)); // down
		}
		if (x > 0) {
			limit.add(new Node(new Vector2(x - 1, y), col, row)); // left
		}
		if (x < col) {
			limit.add(new Node(new Vector2(x + 1, y), col, row)); // right
		}
		return limit;
	}
