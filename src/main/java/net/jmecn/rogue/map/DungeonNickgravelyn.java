...
public class DungeonNickgravelyn extends MapCreator {
	...
	private Vector2 findNewDoorLocation(Room room1, Room room2) {
		Vector2 doorPos = new Vector2(-1, -1);
	...
	private void addStairs(int type) {
        ...
        Vector2 loc = candidates.get(random(candidates));
	...
	private boolean areRoomsConnected(Room room1, Room room2) {
		...
		List<Vector2> doors = room1.getDoorLocations();
		for (int i = 0; i < doors.size(); i++) {
			Vector2 d = doors.get(i);
	...
	private static class Room extends Rect {
		...
		List<Vector2> getDoorLocations() {
			List<Vector2> doors = new ArrayList<Vector2>();
