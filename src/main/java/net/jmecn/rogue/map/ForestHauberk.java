...
public class ForestHauberk extends MapCreator {
	...
	public void create() {
		...
		LinkedList<Vector2> meadows = new LinkedList<Vector2>();
		for (int i = 0; i < numMeadows; i++) {
			int x = nextInt(width);
			int y = nextInt(height);
			meadows.add(new Vector2(x, y));
		}
		...
		for (int i = 0; i < voronoiIterations; i++) {
			...
			List<List<Vector2>> regions = new ArrayList<List<Vector2>>(numMeadows);
			for (int j = 0; j < numMeadows; j++) {
				regions.add(new ArrayList<Vector2>());
			}
			...
			for (int y = 0; y < height; y++) {
				for (int x = 0; x < width; x++) {
					...
					regions.get(nearest).add(new Vector2(x, y));
				}
			}
			...
			for (int j = 0; j < numMeadows; j++) {
				List<Vector2> region = regions.get(j);
				int len = region.size();
		...
		List<Vector2> connected = new ArrayList<Vector2>(numMeadows);
		
		// the root node
		connected.add(meadows.removeLast());
		
		while (!meadows.isEmpty()) {
			Vector2 bestFrom = null;
			Vector2 bestTo = null;
			int bestDistance = -1;
			...
			for(int p=0; p<cLen; p++) {
				Vector2 from = connected.get(p);
				for(int q=0; q<mLen; q++) {
					// skip the same point
					if (q == p) continue;
					Vector2 to = meadows.get(q);
	...
	private void carvePath(Vector2 from, Vector2 to) {
	...
	private void carveCircle(Vector2 center, int radius) {
		// bounds
